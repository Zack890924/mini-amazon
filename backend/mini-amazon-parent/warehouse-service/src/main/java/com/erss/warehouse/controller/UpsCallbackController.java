...
import com.erss.warehouse.service.MetricsService;
...
private ResponseEntity<Map<String, Object>> handleTruckArrived(Map<String, Object> req) {
    try {
        ...
        log.info("=== UPS truck_arrived: truckId={}, warehouseId={} ===", truckId, warehouseId);

        // Use SELECT FOR UPDATE to claim a ready package atomically
        // This prevents concurrent truck_arrived requests from claiming the same package
        List<PackageOperation> ready =
                packageOperationRepository.findAndLockFirstReadyPackage();

        if (ready.isEmpty()) {
            log.info("No packages ready for loading");
            return ResponseEntity.ok(Map.of(
                    "action",         "truck_arrived_response",
                    "in_response_to", "ups",
                    "status",         "success",
                    "message",        "No packages ready for truck " + truckId
            ));
        }

        // Take the first package (already locked by SELECT FOR UPDATE)
        PackageOperation operation = ready.get(0);
        Long pkgId = operation.getPackageId();
        log.info("truck_arrived: claimed package {} for truck {} (locked via SELECT FOR UPDATE)",
                 pkgId, truckId);

        // Notify UPS to load this package
        ...
        if (upsResp!=null && "success".equals(upsResp.get("status"))) {
            // UPS confirmed, proceed with loading
            ...
            log.info("✓ Package {} successfully loaded onto truck {} (UPS confirmed)", pkgId, truckId);
        } else {
            log.warn("UPS load rejected or no response for package {}: {}", pkgId, upsResp);
            // Note: package remains locked until transaction commits/rolls back
        }
        ...
        return ResponseEntity.ok(Map.of(
                ...
                "in_response_to", "ups",
                "status",         "success",
                "message",        "Truck " + truckId + " arrival processed, package " + pkgId
        ));
    } catch (Exception e) {
        log.error("handleTruckArrived error: {}", e.getMessage(), e);
...
//    }
    ...
    private ResponseEntity<Map<String, Object>> handlePackageDelivered(Map<String, Object> request) {
        try{
            ...
            log.info("=== UPS package_delivered: packageId={}, truckId={}, location=({},{}) ===",
            ...
            // STATE MACHINE VALIDATION: Check if package has been loaded
            // This prevents out-of-order callback processing
            // (e.g., delivered arrives before truck_arrived)
            Optional<PackageOperation> loadOp =
                packageOperationRepository.findByPackageIdAndOperation(packageId, "LOAD");

            if (loadOp.isEmpty() || !"COMPLETED".equals(loadOp.get().getStatus())) {
                // Package not loaded yet - callback is out of order
                log.warn("⚠ Package {} received delivered callback, but LOAD operation not found or not COMPLETED. " +
                         "Callback out of order. Current LOAD status: {}",
                         packageId,
                         loadOp.map(PackageOperation::getStatus).orElse("NOT_FOUND"));

                // Return 409 Conflict to tell UPS to retry later
                return ResponseEntity.status(HttpStatus.CONFLICT).body(Map.of(
                        "action", "package_delivered_response",
                        "in_response_to", "ups",
                        "status", "error",
                        "message", "Package not loaded yet. Please retry after loading is confirmed.",
                        "reason", "state_machine_violation"
                ));
            }

            // Check if already delivered (idempotency)
            Optional<PackageOperation> existingDelivered =
                packageOperationRepository.findByPackageIdAndOperation(packageId, "DELIVERED");

            if (existingDelivered.isPresent() && "COMPLETED".equals(existingDelivered.get().getStatus())) {
                log.info("Package {} already delivered (duplicate callback), returning success (idempotent)", packageId);
                return ResponseEntity.ok(Map.of(
                        "action", "package_delivered_response",
                        "in_response_to", "ups",
                        "status", "success",
            ...
            // State machine valid - create DELIVERED operation
            PackageOperation op = new PackageOperation();
            op.setPackageId(packageId);
            ...
            metricsService.trackStateTransition("package_operation", null, "DELIVERED");
            packageOperationRepository.save(op);

            log.info("✓ Package {} delivered successfully: truckId={}, location=({},{})",
                    packageId, truckId, deliveryX, deliveryY);
            ...
            log.error("Error processing package_delivered: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of(
                    "action", "package_delivered_response",
                    "in_response_to", "ups",
                    "status", "error",
